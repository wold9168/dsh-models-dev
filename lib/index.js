...
var ModelsSyncSectionSchema = Schema.object({
  ...
  matching: Schema.object({
    ...
    policy: Schema.union(["mode", "max", "min"]).default("mode")
...
function findIn(perProvider, model, norm) {
  ...
  return hit === void 0 ? void 0 : perProvider.get(hit);
}
function entryOf(index, pid, model, norm) {
  const entry = findIn(index.get(pid), model, norm);
  if (entry === void 0) return void 0;
  return {
    contextWindow: entry.contextWindow,
    ...entry.maxOutput === void 0 ? {} : { maxTokens: entry.maxOutput },
    matchedProvider: pid
  };
}
function aggregate(candidates, policy) {
  if (policy === "max") {
  ...
  for (const group of byValue.values()) {
    if (bestGroup === void 0 || group.length > bestGroup.length) bestGroup = group;
...
function resolveContext(index, provider, model, config) {
  ...
  return {
    ...
    ...chosen.maxOutput === void 0 ? {} : { maxTokens: chosen.maxOutput },
    kind: "match",
...
function planNamespace(writes, skipped, unresolved, provider, models, index, matching) {
...
function planSync(input, index, matching) {
  const writes = [];
  const skipped = [];
  const unresolved = [];
  if (input.llmPiAi?.providers !== void 0) {
...
function buildUpdatePatch(input, writes) {
...
async function syncViaSettings(settings, config, options = {}) {
  ...
  if (patch !== void 0) {
    try {
      ...
      emit?.({ ts: (/* @__PURE__ */ new Date()).toISOString(), level: "info", message: `\u5199\u56DE llm-pi-ai\uFF1A${planned.writes.length} \u6761 contextWindow` });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      errors.push(`models-sync: \u5199\u56DE llm-pi-ai \u5931\u8D25\uFF1A${message}`);
      emit?.({ ts: (/* @__PURE__ */ new Date()).toISOString(), level: "error", message: `\u5199\u56DE\u5931\u8D25 llm-pi-ai: ${message}` });
    }
  }
  return {
...
var ModelsSyncRuntime = class {
  ...
  async loadIndex(section) {
    ...
    this.cache = { index, source, sourceModels: countModels(index), fetchedAt: (/* @__PURE__ */ new Date()).toISOString() };
    return this.cache;
  }
};
...
var policySchema = z.enum(["mode", "max", "min"]);
var nsSchema = z.literal("llm-pi-ai");
var resolveKindSchema = z.enum(["exact", "alias", "preferred", "match"]);
