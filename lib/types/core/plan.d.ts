...
/** 生成同步计划（仅 llm-pi-ai）。 */
export declare function planSync(input: PlanInput, index: ProviderIndex, matching: MatchingConfig): PlanResult;
/**
 * 由计划写出一份 `settings.update` 合并补丁：对象深合并、数组整体替换，所以
 * 补丁里被触碰的 provider 只携带替换后的 `models`，其它字段与命名空间不动。
 */
export declare function buildUpdatePatch(input: PlanInput, writes: PlannedWrite[]): Record<string, unknown> | undefined;
